...
import (
	...
	config "github.com/Calmantara/go-user/lib/setup/config"
...
func openPostgresConnection(param PostgresParam) (gormDB *gorm.DB) {
	...
	for i := 0; i <= 3; i++ {
		gormDB, err = gorm.Open(
			postgres.Open(param.postgresConfigString()),
			&gorm.Config{Logger: pgLog})
		if err == nil {
			break
		}
		log.Printf("error to connect to databse: %+v", err)
